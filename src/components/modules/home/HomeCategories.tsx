      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-gray-900 dark:text-white mb-5 tracking-tight">
            Explore by <span className="bg-gradient-to-r from-orange-500 to-red-500 dark:from-orange-400 dark:to-red-500 bg-clip-text text-transparent">Category</span>
          </h2>
          <p className="text-gray-600 dark:text-gray-400 text-lg md:text-xl font-medium leading-relaxed max-w-2xl mx-auto">
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="h-[180px] bg-slate-100 dark:bg-white/5 animate-pulse rounded-2xl border border-black/5 dark:border-white/10" />
            ))}
          </div>
        ) : categories.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              ...
              return (
                <Link href={`/categories/${cat.slug}`} key={cat.id} className="no-underline group">
                  <div className={`relative p-6 rounded-2xl bg-gradient-to-br ${colorClass} border backdrop-blur-sm transition-all duration-300 hover:scale-[1.03] hover:shadow-xl cursor-pointer`}>
                    <div className={`w-14 h-14 rounded-xl bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform ${iconColor}`}>
                      <IconComponent className="w-7 h-7" />
                    </div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1 group-hover:text-gray-900 dark:group-hover:text-white/90">{cat.name}</h3>
          ...
          <div className="text-center py-20 bg-slate-50 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded-3xl">
            ...
            <p className="text-gray-500 dark:text-gray-400">Categories will appear here once available.</p>
          </div>
        )}
      </div>
    </section>
